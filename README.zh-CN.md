...
会自动转换为其原生的 `$review-patch` 形式。没有成员前缀的 `/review`、`/team` 等仍是
Asterline 自己的命令。
...
`/review`、`/plan`（别名 `/lead`）和 `/roundtable` 是由运行时状态机驱动的协作
模式，而不是普通聊天回合。审阅模式中 builder 负责实现，reviewer 必须以结构化的
`@@review` verdict 作答：`approve` 结束运行（可选自动验证），`request_changes`
会把反馈回传给 builder 继续迭代，次数受 `max_iterations` 约束。领队模式由 leader
规划带负责人的任务清单，Asterline 自动派发给各负责人，完成后进入同样的审阅环节。
圆桌模式进行 N 轮讨论，每位成员都能看到其他人的观点，最后可选由主持人综合结论。

谁实现、谁审阅、谁领队、谁主持，都可以在 `team.json` 的 `modes` 配置或命令行内
（`/review reviewer=claude builder=codex …`）自定义。`/runs` 会显示模式相位、
迭代预算、步骤负责人、verdict 时间线、阻塞和下一条建议命令。
...
常见检查。`/workflow` 保留原有的提示词驱动协调方式作为回退路径。
...
再按 `s` 保存。按 `e` 可手动输入（Agy 目前需要手动输入），输入 `auto` 可取消显式绑定。
...
| `@all <message>`       | 向全队广播                        |
| `/review <task>`       | builder 实现，reviewer 出 verdict |
| `/plan <goal>`         | 领队规划任务清单，由引擎派发      |
| `/roundtable <topic>`  | 多 Agent 圆桌讨论                 |
...
Asterline 当前版本为 `0.1.2`，仍在积极开发。带版本标签的提交会通过 GitHub
