...
用 `↑`、`↓` 高亮模式，按 `Enter` 进入可选字段，再按 `s` 选择模式并把 pending
覆盖应用到本对话。如果已经确定模式，`/mode <名称>` 仍是直接切换的快捷方式。选择
`review`、`plan`、`brainstorm` 或 `team` 后，直接输入任务即可，不需要 `@成员`；
新的 `normal` 对话仍需用 `@builder` 这类显式目标：
...
Codex 工具询问默认自动通过，除非加上 `--manual-approvals`。Asterline 不会在所选后端之外再提供一层进程沙箱。放宽权限前，请阅读[审批与工具控制](docs/approvals.md)。
